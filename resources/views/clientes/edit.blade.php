...
<div class="max-w-2xl" x-data="consultaRuc()">
    <form action="{{ route('clientes.update', $cliente) }}" method="POST" class="bg-white shadow-sm rounded-lg p-6 space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <div class="flex gap-2">
                    <input type="text" name="ruc" x-model="ruc" value="{{ old('ruc', $cliente->ruc) }}" required
                           maxlength="11"
        ...
        <div>
            ...
            <input type="text" name="razon_social" x-model="razonSocial" value="{{ old('razon_social', $cliente->razon_social) }}" required
                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="0.01" name="monto" value="{{ old('monto', $cliente->monto) }}"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Fecha Vencimiento</label>
                <input type="date" name="fecha_vencimiento" value="{{ old('fecha_vencimiento', $cliente->fecha_vencimiento?->format('Y-m-d')) }}"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
        </div>
        ...
        <div>
            ...
            <select name="tipo_servicio"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                <option value="mensual" {{ $cliente->tipo_servicio == 'mensual' ? 'selected' : '' }}>Mensual</option>
                <option value="trimestral" {{ $cliente->tipo_servicio == 'trimestral' ? 'selected' : '' }}>Trimestral</option>
                <option value="semestral" {{ $cliente->tipo_servicio == 'semestral' ? 'selected' : '' }}>Semestral</option>
                <option value="anual" {{ $cliente->tipo_servicio == 'anual' ? 'selected' : '' }}>Anual</option>
            </select>
        </div>
